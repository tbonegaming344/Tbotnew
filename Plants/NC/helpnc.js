...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "Night Cap Decks" : `Night Cap ${name} Decks`)
    .setDescription(
      isAll
        ? `All Night Cap decks in Tbot are:${description}`
        : `My ${name} decks for Night Cap are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("Red")
    .setFooter({
      text: isAll
        ? `Night Cap has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Night Cap has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!rows || rows.length === 0) {
      return message.channel.send("No Night Cap decks found in the database.");
    }
