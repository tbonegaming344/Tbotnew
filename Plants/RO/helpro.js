...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "Rose Decks" : `Rose ${name} Decks`)
    .setDescription(
      isAll
        ? `All Rose decks in Tbot are:${description}`
        : `My ${name} decks for Rose are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("White")
    .setFooter({
      text: isAll
        ? `Rose has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Rose has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
