/**
 * @description Gets all static custom statuses (excluding ones with variables)
 * @returns {Array<string>} Array of static custom status strings
 */
function getStaticCustomStatuses() {
  const customStatus = [
    ///247 IQ
  ...
  // Filter out statuses that contain variables (template literals with ${})
  return customStatus.filter(status => !status.includes("${"));
}

module.exports = getStaticCustomStatuses;
