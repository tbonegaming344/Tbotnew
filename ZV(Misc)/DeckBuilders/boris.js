...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "Boris Decks" : `Boris ${name} Decks`)
    .setDescription(
      isAll
        ? `All Boris decks in Tbot are:${description}`
        : `My ${name} decks for Boris are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("White")
    .setFooter({
      text: isAll
        ? `Boris has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Boris has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    const [rows] =
      await db.query(`SELECT * FROM ccdecks where creator like '%Boris%'
      union all select * from bfdecks where creator like '%Boris%' 
      union all select * from bcdecks where creator like '%Boris%'`);
    if (!rows || rows.length === 0) {
      return message.channel.send("No Boris decks found in the database.");
    }
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          .setTitle("Boris Decks")
          .setDescription(
            `To view the Boris decks please select an option from the select menu below!\nNote: Boris has ${normalized.length} total decks in Tbot`
          )
          .setColor("White")
          .setThumbnail(thumb),
      ],
