...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(
      isAll
        ? "Bowling Bulb Enjoyer Decks"
        : `Bowling Bulb Enjoyer ${name} Decks`
    )
    .setDescription(
      isAll
        ? `All Bowling Bulb Enjoyer decks in Tbot are:${description}`
        : `My ${name} decks for Bowling Bulb Enjoyer are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("Orange")
    .setFooter({
      text: isAll
        ? `Bowling Bulb Enjoyer has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Bowling Bulb Enjoyer has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    const [rows] = await db.query(
      "SELECT * FROM bfdecks where creator like '%Bowling Bulb Enjoyer%' union all select * from ctdecks where creator like '%Bowling Bulb Enjoyer%'"
    );
    if (!rows || rows.length === 0) {
      return message.channel.send(
        "No Bowling Bulb Enjoyer decks found in the database."
      );
    }
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          .setTitle("Bowling Bulb Enjoyer Decks")
          .setDescription(
            `To view the Bowling Bulb Enjoyer decks please select an option from the select menu below! \n Note: Bowling Bulb Enjoyer has ${normalized.length} total decks in Tbot`
          )
          .setColor("Orange")
          .setThumbnail(thumb),
      ],
