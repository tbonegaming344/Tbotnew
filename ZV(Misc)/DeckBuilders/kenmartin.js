...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "Ken Martin Decks" : `Ken Martin ${name} Decks`)
    .setDescription(
      isAll
        ? `All Ken Martin decks in Tbot are:${description}`
        : `My ${name} decks for Ken Martin are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("#E5AA70")
    .setFooter({
      text: isAll
        ? `Ken Martin has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Ken Martin has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    const [rows] =
      await db.query(`SELECT * FROM rbdecks where creator like '%ken martin%'
      union all select * from pbdecks where creator like '%ken martin%'
      union all select * from hgdecks where creator like '%ken martin%'`);
    if (!rows || rows.length === 0) {
      return message.channel.send("No Ken Martin decks found in the database.");
    }
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `To view the Ken Martin decks please select an option from the select menu below!\nNote: Ken Martin has ${normalized.length} total decks in Tbot`
          )
          .setColor("#E5AA70")
          .setThumbnail(thumb),
      ],
