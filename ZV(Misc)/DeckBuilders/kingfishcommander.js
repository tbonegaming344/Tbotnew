...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(
      isAll ? "King Fish Commander Decks" : `King Fish Commander ${name} Decks`
    )
    .setDescription(
      isAll
        ? `All King Fish Commander decks in Tbot are:${description}`
        : `My ${name} decks for King Fish Commander are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("Green")
    .setFooter({
      text: isAll
        ? `King Fish Commander has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `King Fish Commander has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    const [rows] = await db.query(`SELECT * FROM bfdecks 
      where creator like '%king fish commander%'
      union all select * from gsdecks where creator like '%king fish commander%'
      union all select * from ntdecks where creator like '%king fish commander%'
      union all select * from rbdecks where creator like '%king fish commander%'`);
    if (!rows || rows.length === 0) {
      return message.channel.send(
        "No King Fish Commander decks found in the database."
      );
    }
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          .setTitle("King Fish Commander Decks")
          .setDescription(
            `To view the King Fish Commander decks please select an option from the select menu below!\nNote: King Fish Commander has ${normalized.length} total decks in Tbot`
          )
          .setColor("Green")
          .setThumbnail(thumb),
      ],
