...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(
      isAll ? "Make Me A Coffee Decks" : `Make Me A Coffee ${name} Decks`
    )
    .setDescription(
      isAll
        ? `All Make Me A Coffee decks in Tbot are:${description}`
        : `My ${name} decks for Make Me A Coffee are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("White")
    .setFooter({
      text: isAll
        ? `Make Me A Coffee has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Make Me A Coffee has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          .setTitle("Make Me A Coffee Decks")
          .setDescription(
