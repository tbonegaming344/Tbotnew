...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "Maker Decks" : `Maker ${name} Decks`)
    .setDescription(
      isAll
        ? `All Maker decks in Tbot are:${description}`
        : `My ${name} decks for Maker are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("Orange")
    .setFooter({
      text: isAll
        ? `Maker has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Maker has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    const [rows] =
      await db.query(`SELECT * FROM rbdecks where creator like '%Maker%'
      union all SELECT * FROM imdecks where creator like '%Maker%'`);
    if (!rows || rows.length === 0) {
      return message.channel.send("No Maker decks found in the database.");
    }
    ...
    const select = new StringSelectMenuBuilder()
      .setCustomId("select")
      .setPlaceholder("Select an option below to view Maker decklists")
      .addOptions(
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          .setTitle("Maker Decks")
          .setDescription(
            `To view the Maker decks please select an option from the select menu below!\nNote: Maker has ${normalized.length} total decks in Tbot`
          )
          .setColor("Orange")
          .setThumbnail(thumb),
      ],
