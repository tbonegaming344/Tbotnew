...
module.exports = {
  ...
  run: async (client, message, args) => {
    const color = "#6679d9";
    const [rows] = await db.query("SELECT * FROM smdecks where creator like '%pvzfurmade%' union all SELECT * FROM pbdecks where creator like '%pvzfurmade%'");
    if (!rows || rows.length === 0) {
      return message.channel.send(
        "No PvZ Furmade decks found in the database."
      );
    }
