...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "PvzFurMade Decks" : `PvzFurMade ${name} Decks`)
    .setDescription(
      isAll
        ? `All PvzFurMade decks in Tbot are:${description}`
        : `My ${name} decks for PvzFurMade are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("#6679d9")
    .setFooter({
      text: isAll
        ? `PvzFurMade has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `PvzFurMade has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          .setTitle("PvzFurMade Decks")
          .setDescription(
            `To view the PvzFurMade decks please select an option from the select menu below!\nNote: PvzFurMade has ${normalized.length} total decks in Tbot`
          )
          .setColor("#6679d9")
          .setThumbnail(thumb),
      ],
