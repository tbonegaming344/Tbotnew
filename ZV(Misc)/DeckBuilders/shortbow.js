...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "Shortbow Decks" : `Shortbow ${name} Decks`)
    .setDescription(
      isAll
        ? `All Shortbow decks in Tbot are:${description}`
        : `My ${name} decks for Shortbow are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("#6679d9")
    .setFooter({
      text: isAll
        ? `Shortbow has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Shortbow has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    const [rows] =
      await db.query(`SELECT * FROM sbdecks where creator like '%shortbow%' 
      union all select * from ccdecks where creator like '%shortbow%'
      union all select * from sfdecks where creator like '%shortbow%'
      union all select * from rodecks where creator like '%shortbow%'
      union all select * from gsdecks where creator like '%shortbow%'
      union all select * from wkdecks where creator like '%shortbow%'
      union all select * from czdecks where creator like '%shortbow%'
      union all select * from spdecks where creator like '%shortbow%'
      union all select * from ctdecks where creator like '%shortbow%'
      union all select * from bcdecks where creator like '%shortbow%'
      union all select * from gkdecks where creator like '%shortbow%'
      union all select * from ncdecks where creator like '%shortbow%'
      union all select * from hgdecks where creator like '%shortbow%'
      union all select * from zmdecks where creator like '%shortbow%'
      union all select * from smdecks where creator like '%shortbow%'
      union all select * from ifdecks where creator like '%shortbow%'
      union all select * from rbdecks where creator like '%shortbow%'
      union all select * from ebdecks where creator like '%shortbow%'
      union all select * from bfdecks where creator like '%shortbow%'
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          .setTitle("ShortBow Decks")
          .setDescription(
            `To view the ShortBow decks please select an option from the select menu below!\nNote: ShortBow has ${normalized.length} total decks in Tbot`
          )
          .setColor("#6679d9")
          .setThumbnail(thumb),
      ],
