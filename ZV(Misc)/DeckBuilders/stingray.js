...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "Sting Ray Decks" : `Sting Ray ${name} Decks`)
    .setDescription(
      isAll
        ? `All Sting Ray decks in Tbot are:${description}`
        : `My ${name} decks for Sting Ray are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("#d07be0")
    .setFooter({
      text: isAll
        ? `Sting Ray has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Sting Ray has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    const select = new StringSelectMenuBuilder()
      .setCustomId("select")
      .setPlaceholder("Select an option below to view stingray's decklists")
      .addOptions(
    ...
    const [rows] =
      await db.query(`SELECT * FROM czdecks where creator like '%stingray%'
            union all select * from ebdecks where creator like '%stingray%'`);
    if (!rows || rows.length === 0) {
      return message.channel.send("No Stingray decks found in the database.");
    }
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          .setTitle("Stingray Decks")
          .setDescription(
            `To view the Stingray decks please select an option from the select menu below!\nNote: Stingray has ${normalized.length} total decks in Tbot`
          )
          .setColor("#d07be0")
          .setThumbnail(thumb),
      ],
