...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "Sushi Decks" : `Sushi ${name} Decks`)
    .setDescription(
      isAll
        ? `All Sushi decks in Tbot are:${description}`
        : `My ${name} decks for Sushi are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("#de8d5d")
    .setFooter({
      text: isAll
        ? `Sushi has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Sushi has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          .setTitle("Sushi Decks")
          .setDescription(
            `To view the Sushi decks please select an option from the select menu below!\nNote: Sushi has ${normalized.length} total decks in Tbot`
          )
          .setColor("#de8d5d")
          .setThumbnail(thumb),
      ],
