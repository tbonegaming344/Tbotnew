...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "Tbone Decks" : `Tbone ${name} Decks`)
    .setDescription(
      isAll
        ? `All Tbone decks in Tbot are:${description}`
        : `My ${name} decks for Tbone are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("DarkGold")
    .setFooter({
      text: isAll
        ? `Tbone has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Tbone has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    const [rows] = await db.query(`SELECT * FROM ebdecks
      where creator like '%Tbone%'
      union all select * from smdecks where creator like '%Tbone%'`);
    if (!rows || rows.length === 0) {
      return message.channel.send("No Tbone decks found in the database.");
    }
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          .setTitle("Tbone Decks")
          .setDescription(
            `To view the Tbone decks please select an option from the select menu below!\nNote: Tbone has ${normalized.length} total decks in Tbot`
          )
          .setColor("DarkGold")
          .setThumbnail(thumb),
      ],
