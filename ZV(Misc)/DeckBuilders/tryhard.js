...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "Tryhard Decks" : `Tryhard ${name} Decks`)
    .setDescription(
      isAll
        ? `All Tryhard decks in Tbot are:${description}`
        : `My ${name} decks for Tryhard are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("#1483C9")
    .setFooter({
      text: isAll
        ? `Tryhard has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Tryhard has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    const [rows] = await db.query(`SELECT * FROM rbdecks
      where creator like '%TryHard%'
      union all select * from hgdecks where creator like '%TryHard%'`);
    if (!rows || rows.length === 0) {
      return message.channel.send("No TryHard decks found in the database.");
    }
    ...
    const m = await message.channel.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `To view the TryHard decks please select an option from the select menu below!\nNote: TryHard has ${normalized.length} total decks in Tbot`
          )
          .setColor("#1483C9")
          .setThumbnail(thumb),
      ],
