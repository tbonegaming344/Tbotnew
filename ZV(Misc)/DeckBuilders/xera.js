    ...
    run: async(client, message, args) => {
      const select = new StringSelectMenuBuilder()
          ...
          const row = new ActionRowBuilder()
                ...
                let toBuildLadderString = ladderdecks.map(deck => `\n<@1043528908148052089> **${deck}**`).join('');
                ...
                let toBuildMeme = memedecks.map(deck => `\n<@1043528908148052089> **${deck}**`).join('');
                ...
                let toBuildComboString = combodecks.map(deck => `\n<@1043528908148052089> **${deck}**`).join('');
                ...
                let toBuildControlString = controldecks.map(deck => `\n<@1043528908148052089> **${deck}**`).join('');
                ...
                let toBuildMidString = midrangedecks.map(deck => `\n<@1043528908148052089> **${deck}**`).join('');  
        let [result] = await db.query(`SELECT gomorrah, gps, himps, lasersnap, startron, toyotacontrolla, feastmech, brady, watersports 
            FROM bfdecks bf 
            inner join ctdecks ct on (bf.deckinfo = ct.deckinfo)
            inner join ncdecks nc on (bf.deckinfo = nc.deckinfo)
            inner join czdecks cz on (bf.deckinfo = cz.deckinfo)
            inner join hgdecks hg on (bf.deckinfo = hg.deckinfo)
            inner join ntdecks nt on (bf.deckinfo = nt.deckinfo)
            inner join zmdecks zm on (bf.deckinfo = zm.deckinfo)`)
        ...
        let toBuildString = decks.map(deck => `\n<@1043528908148052089> **${deck}**`).join('');  
...
Note: ${user.displayName} has ${decks.length} total decks in Tbot`,
    ...
    .setDescription(`My Ladder Decks made by ${user.displayName} are ${toBuildLadderString}`)
    .setFooter({
