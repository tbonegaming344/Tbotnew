...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(
      isAll ? "Captain Combustible Decks" : `Captain Combustible ${name} Decks`
    )
    .setDescription(
      isAll
        ? `All Captain Combustible decks in Tbot are:${description}`
        : `My ${name} decks for Captain Combustible are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("Red")
    .setFooter({
      text: isAll
        ? `Captain Combustible has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Captain Combustible has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const [rows] = await db.query("SELECT * FROM ccdecks");
    ...
    const [heroresult] = await db.query(
      `select captaincombustible from plantheroes`
    );
    const cc = new EmbedBuilder()
      .setThumbnail(`${heroresult[2].captaincombustible}`)
      .setTitle(`${heroresult[5].captaincombustible}`)
      .setDescription(`${heroresult[0].captaincombustible}`)
      .setColor("Red")
      .addFields(
        {
          name: "Superpowers",
          value: `${heroresult[4].captaincombustible}`,
          inline: true,
        },
        {
          name: "Set-Rarity",
    ...
    const helpccButton = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("helpcc")
    ...
    collector.on("collect", async (i) => {
      try {
        // pressed the initial helpcc button -> show the select menu (same UI helpcc uses)
        if (i.customId === "helpcc" && i.isButton()) {
          ...
          await i.update({
            embeds: [
              createHelpEmbed(
                ...
                `Select a category below to view the Captain Combustible decks — Captain Combustible has ${normalized.length} total decks.`,
                thumb
              ),
            ],
