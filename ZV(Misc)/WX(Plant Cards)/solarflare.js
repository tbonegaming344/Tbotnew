...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(isAll ? "Solar Flare Decks" : `Solar Flare ${name} Decks`)
    .setDescription(
      isAll
        ? `All Solar Flare decks in Tbot are:${description}`
        : `My ${name} decks for Solar Flare are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("Red")
    .setFooter({
      text: isAll
        ? `Solar Flare has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Solar Flare has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const [rows] = await db.query("SELECT * FROM sfdecks");
    if (!rows || rows.length === 0)
      return message.channel.send(
        "No Solar Flare decks found in the database."
      );
    ...
    const helpsfButton = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("helpsf")
    ...
    collector.on("collect", async (i) => {
      try {
        // pressed the initial helpsf button -> show the select menu (same UI helpsf uses)
        if (i.customId === "helpsf" && i.isButton()) {
          const select = new StringSelectMenuBuilder()
            .setCustomId("select")
            .setPlaceholder(
              "Select an option below to view Solar Flare's decklists"
            )
            .addOptions(
          ...
          await i.update({
            embeds: [
              createHelpEmbed(
                "Solar Flare Decks",
                `Select a category to view Solar Flare Decks — Solar Flare has ${normalized.length} total decks.`,
                thumb
              ),
            ],
        ...
        console.error("solar flare collector error:", err);
