...
function createCategoryEmbed(name, deckNames, total, thumbnail) {
  ...
  return new EmbedBuilder()
    .setTitle(
      isAll ? "Electric Bogaloo Decks" : `Electric Bogaloo ${name} Decks`
    )
    .setDescription(
      isAll
        ? `All Electric Bogaloo decks in Tbot are:${description}`
        : `My ${name} decks for Electric Bogaloo are: ${description}`
    )
    .setThumbnail(thumbnail)
    .setColor("Blue")
    .setFooter({
      text: isAll
        ? `Electric Bogaloo has ${total} total decks in Tbot\nPlease click on the buttons below to navigate through the decks.`
        : `Electric Bogaloo has ${total} total ${name} decks in Tbot\nPlease click on the buttons below to navigate through the decks.`,
    });
}
...
module.exports = {
  ...
  run: async (client, message, args) => {
    const [rows] = await db.query("SELECT * FROM ebdecks");
    if (!rows || rows.length === 0)
      return message.channel.send(
        "No Electric Boogaloo decks found in the database."
      );
    ...
    const thumb = "https://pbs.twimg.com/media/C2utROCXUAQh7aZ.png";
    ...
    const helpebButton = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("helpeb")
    ...
    collector.on("collect", async (i) => {
      try {
        if (i.customId === "helpeb" && i.isButton()) {
          ...
          await i.update({
            embeds: [
              createHelpEmbed(
                ...
                `Select a category — Electric Boogaloo has ${normalized.length} total decks.`,
                thumb
              ),
            ],
        ...
        console.error("Electric Boogaloo collector error:", err);
